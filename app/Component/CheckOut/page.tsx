...
function CheckoutContent() {
  ...
  const loadCheckoutData = async () => {
    try {
      ...
      const parsedCheckoutData = storedCheckoutData ? JSON.parse(storedCheckoutData) : null;
      ...
      const total = currentProducts.reduce((sum, p) => sum + (parseFloat(p.price) * (p.quantity || 1)), 0);
      setCartTotal(total);
  ...
  const handleRazorpayOrder = async () => {
    ...
    try {
      ...
      const options = {
        ...
        handler: async function (response: any) {
          ...
          try {
            ...
            router.push(`/Component/Orders/OrderFailure?paymentId=${response.razorpay_payment_id}&error=${encodeURIComponent(errorMsg)}`);
      ...
      rzp.on('payment.failed', function (response: any) {
        ...
        router.push(`/Component/Orders/OrderFailure?error=${encodeURIComponent(response.error.description)}`);
        setSubmitting(false);
      });
      ...
      router.push(`/Component/Orders/OrderFailure?error=${encodeURIComponent(errorMsg)}`);
      setSubmitting(false);
    }
  };
